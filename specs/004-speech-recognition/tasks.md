...
- [ ] T002 Implement `AsrEngine` struct with field `ctx: Arc<Mutex<WhisperContext>>`, derive `Clone` via manual impl (Arc::clone per research R-003), and implement `AsrEngine::new(model_path: &Path, use_gpu: bool) -> Result<AsrEngine>` — create `WhisperContextParameters`, call `use_gpu()`, load model via `WhisperContext::new_with_params()`, wrap in `Arc::new(Mutex::new())`. Add `///` doc comments on struct and `new()`. File: `crates/vox_core/src/asr.rs`
